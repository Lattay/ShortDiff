...
# Complexité spatiale Omega(M+1 + (M+1)*(M+N))
#                 distance --^      ^-- chemins
...
def differ(old, new):
    ...
    for n in range(N+1):
        ...
        for j in range(M, -1, -1):
            path_mat[j+1] = path_mat[j]
            dist_mat[j+1] = dist_mat[j]
    
    return dist_mat[M+1], path_mat[M+1]
